...
running = False
TraceEvent = namedtuple('TraceEvent', 'cat name pid tid ts dur tdur ph')
...
def json_output():
    ...
    with open('trace.json', 'w') as f:
        ...
        for v in meta_thread_maps.values():
            f.write(json.dumps(v._asdict()))
            f.write(',')

        for v in meta_proc_maps.values():
            f.write(json.dumps(v._asdict()))
            f.write(',')
        f.write(tail)
        f.write(tail)
...
def print_event(cpu, data, size):
    ...
    if args.previous:
        if event.tid not in meta_thread_maps:
            item = {"name": event.task.decode('utf-8', 'replace')}
            meta_data = NameMeta("__metadata", item, "thread_name", 0, event.tid, 0, 'M')
            meta_thread_maps[event.tid] = meta_data

        if event.pid not in meta_proc_maps:
            try:
                p = psutil.Process(event.pid)
                item = {"name": p.name()}
                meta_data = NameMeta("__metadata", item, "process_name", event.pid, 0, 0, 'M')
                meta_proc_maps[event.pid] = meta_data
            except psutil.NoSuchProcess as e:
                pass

        if event.prev_pid not in meta_thread_maps:
            item = {"name": event.prev_task.decode('utf-8', 'replace')}
            meta_data = NameMeta("__metadata", item, "thread_name", 0, event.prev_pid, 0, 'M')
            meta_thread_maps[event.prev_pid] = meta_data

        time_us = int((time.time_ns() - start_time) / 1000)
        te = TraceEvent(event.prev_task.decode('utf-8', 'replace'), event.task.decode('utf-8', 'replace'), event.pid, event.tid, time_us, event.delta_us, event.delta_us, 'X')

        try:
            events_queue.put_nowait(te)
...
output_thread = Thread(target=json_output)
running = True
output_thread.start()
